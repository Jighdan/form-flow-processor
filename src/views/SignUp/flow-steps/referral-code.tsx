...
export const StepReferralCode: Step = {
  ...
  Component: ({ moveToNextStep, storeFields, updateStoreFields }) => {
    const { handleSubmit, register, formState } = useForm<Fields>({ defaultValues: storeFields });
    ...
    return (
      <form onSubmit={handleSubmit(onSubmit)}>
        <input {...register("referralCode")} type="text" required placeholder="enter your referral code" />

        <div>
          <button type="button" onClick={onSkip}>Skip</button>
          <button type="submit" disabled={isSubmitButtonDisabled}>Submit Referral Code</button>
        </div>
      </form>
    );
  },
